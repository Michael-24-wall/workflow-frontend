...
export default function InvitationsPanel() {
  ...
  try {
    setLoading(true);
    setError(null);
    console.log('🔄 Loading channel invitations...');
    
    // Use the correct URL with underscore
    const response = await channelService.getMyInvitations();
    console.log('📨 Invitations API response:', response);
    
    // Handle the response format from your Django API
    const invitationsData = response.invitations || response.data || [];
  ...
  const handleAcceptInvitation = async (channelId) => {
    try {
      ...
      setInvitations(prev => prev.filter(inv => inv.id !== channelId));
        ...
        {error ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-400 text-xs mt-1">
              You'll see channel invitations here when they're sent to you
            </p>
          </div>
        ) : (
                  ...
                  <div className="flex-1">
                    <h4 className="font-semibold text-gray-900 flex items-center">
                      ...
                      <span className="ml-2 text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full">
                        {invitation.channel_type}
                      </span>
                    </h4>
                    
                    <p className="text-sm text-gray-600 mt-1">
                      <span className="font-medium">{invitation.invited_by_name}</span> invited you to join this channel
                    </p>
